...
class ImageEditor:
    ...
    def create_zoom_frames(self):
        ...
        # Create internal frames (transformation corners to full image)
        internal_frames = []
        for frame_num in range(num_frames):
            t = 1 - frame_num / num_frames

            current_corners = []
            for (ox, oy), (tx, ty) in zip(original_corners, self.transformation_corners):
                current_x = int(ox + (tx - ox) * t)
                current_y = int(oy + (ty - oy) * t)
                current_corners.append((current_x, current_y))

            coeffs = self.find_coeffs(current_corners, original_corners)
            frame_image = self.output_image.transform(
                self.original_size,
                Image.PERSPECTIVE,
                coeffs,
                Image.Resampling.BICUBIC
            )
            internal_frames.append(frame_image)
        internal_frames.reverse()  # Keep this reverse to maintain correct direction
        
        # Create external frames (full image corners to transformation corners)
        external_frames = []
        for frame_num in range(num_frames):
            t = frame_num / num_frames  # Same timing formula
        ...
        # Combine frames by pasting internal frames onto external frames
        combined_frames = []
        for ext_frame, int_frame in zip(external_frames, internal_frames):
            combined = Image.new("RGBA", self.original_size, (255, 255, 255, 0))
            combined.paste(ext_frame, (0, 0), ext_frame)
           # combined.paste(int_frame, (0, 0), int_frame)
